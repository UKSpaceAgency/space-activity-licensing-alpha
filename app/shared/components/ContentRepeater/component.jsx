...
const ContentRepeater = props => {
  ...
  let tabs = props.commentBlock && props.commentBlock.tabs ? <div className='comment-wrapper'><Heading text='Comments' type='h3' modifiers='heading-medium'/><Tablist>{props.commentBlock.tabs.map((v, i) => <Tab key={i} id={v.id}>{v.label}</Tab>)}</Tablist>{props.commentBlock.tabs.map((v, i) => <TabPanel key={i} id={v.id}>{props.commentBlock && <CommentModule id={props.permalink + v.id} {...props.commentBlock}/>}</TabPanel>)}</div> : null
